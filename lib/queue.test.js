...
function getAllRecords() {
  return getDb()
    ...
    .then(R.sortBy(R.prop('createdDate')));
}
...
describe(__filename, function() {
  let onProcessStub;
  let onFailureStub;
  let onStatusesCheckProcessStub;
      ...
      Object.assign(
        {
          ...
          onStatusesCheckProcess: onStatusesCheckProcessStub
        },
        overrides
      )
    );
  ...
  beforeEach(function() {
    ...
    onStatusesCheckProcessStub = sinon.stub().resolves();
  });
  ...
  describe('#Enqueue', function() {
    it('Should enqueue records', function() {
      ...
      return queue.enqueue({ foo: true, id: 'myId' }).then(function(res) {
        ...
        expect(res).toIncludeKeys(['_id', 'createdDate']);
        expect(res).toInclude({
          //status: 'received',
          data: { foo: true, id: 'myId' }
        });
      });
    });
  });
  ...
  it('Should happy path', function() {
    ...
    return queue
      ...
      .then(function() {
        ...
        expect(onProcessStub.getCall(0).args[0]).toIncludeKey('createdDate');
  ...
  it('Should process only batchSize each time', function() {
    ...
    return Promise.all([queue.enqueue({ foo: 1 }), queue.enqueue({ foo: 2 })])
      ...
      .then(function() {
        ...
        expect(onProcessStub.getCall(1).args[0]).toIncludeKey('createdDate');
  ...
  describe('#Failures in processing', function() {
    ...
    it('Should not let a failure to notify stop processing of other records', function() {
    ...
    it('Should not process any more records if a record fails and continueProcessingOnError is false', function() {
      ...
      return queue
        ...
        .then(function(records) {
          ...
          // expect(records[1]).toInclude({
          //   status: 'received'
          // }); // Second record was not touched
        })
        .then(function() {
          return Promise.delay(51);
        })
  ...
  describe('#Statuses Check', function() {
    it('Should perform a statuses check on collection', function() {
      const queue = createQueue({
        maxRecordAge: 100
      });

      return queue.statusesCheck().then(function() {
        expect(onStatusesCheckProcessStub.callCount).toEqual(1);
      });
    });
  });
    ...
    it('Should fail the record immediately and not reprocess it if fail is called', function() {
      ...
      return queue
        ...
        .then(function() {
          ...
          expect(onFailureStub.callCount).toEqual(1);
          // Make sure the failureReason is available to the fail function
          expect(onFailureStub.getCall(0).args[0]).toInclude({
            status: 'failed',
