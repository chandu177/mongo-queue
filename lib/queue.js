...
const STATUS_CODES = {
  //received: 'received', // 'we dont set received status on cloud payload queue to avoid additional sync cycles'
  duplicate: 'duplicate',
  processed: 'processed',
  failed: 'failed',
  skipped: 'skipped',
...
module.exports = function(opts) {
  ...
  */
  ...
  /**
   * Deletes any records with status=processed or processedDate older than
        ...
        { duplicateOf: { $exists: true } },
        {
          status: {
            $in: [
              ...
              STATUS_CODES.duplicate,
              STATUS_CODES.notified,
              STATUS_CODES.notifyFailure
            ]
          }
        }
      ]
  ...
  function getNextBatch() {
    const statusesForRecords = {
      $or: [
        { status: { $exists: false } },
        { status: { $in: [STATUS_CODES.failed, STATUS_CODES.skipped] } }
      ]
    };
    ...
    const getUnprocessed = {
      $and: [
        statusesForRecords,
        {
          createdDate: { $exists: true } // Can be processed
        }
      ]
    };
  ...
  function prioritizeRecords(records) {
    ...
    if (firstFailed && firstFailed.processedDate > new Date()) {
      ...
      records = records.filter(function(record) {
        if (record.processedDate) {
          return record.processedDate <= new Date();
        }
        if (record.createdDate) {
          return moment(record.createdDate).utc() <= new Date();
        }
      });
  ...
  function processSuccess(record) {
    if (record.status === STATUS_CODES.duplicate) {
      return Promise.resolve(); //we already set duplicate status in DB and send out notification from own implementation
    }
      ...
      return collection.update(
        ...
        },
        {
          ...
          $unset: {
            failureReason: '',
            retryCount: '',
            payload: '' //on success we remove payload to reduce burden on sync
          }
        }
      );
    });
    ...
    return getCollection().then(function(collection) {
      return collection.update(
        ...
        },
        {
          $set: {
            status: STATUS_CODES.failed,
            processedDate: new Date(Date.now() + delay),
      ...
      return collection.update(
        ...
        },
        {
          $set: {
            ...
            processedDate: new Date(Date.now() + utils.getSkipBackoff(skipErr))
          }
        }
      );
    });
